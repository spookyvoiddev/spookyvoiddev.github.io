...
As I’m sure you’ve noticed in my Machinations, I stick pretty heavily to the same nodes and connectors as Machinations does, quite frankly, an _awful_ job of describing how each node works.

However, Machinations does include “documentation” and a plethora of example connections that model out exactly how each node works, and how to make specific systems within Machinations (such as HP regen, cooldowns, reload timers, etc.,) which I commend as many times game design tools simply neglect these elements.

A common trend among game design tools, and specifically engines, is sensory overload. Immediately when opening a tool, your computer’s fan decides to awake from hibernation, you experience lag for a few seconds, and then when everything seems to be settled, you are bombarded with a plethora of options and menus, and you have NO CLUE what they do. These tools are not intuitive in the slightest, and while that may be by design in order to allow for more specific and thorough development, are we not losing many creative ideas that could be quickly prototyped, if they tools were more suited for that sort of development? Does every tool need to be used for grand development?

I appreciate a tool such as Machination, despite its flaws, because it attempts to bridge the gab between the newbie developer and the experienced unity/unreal designer. Machinations allows the designer the freedom to play with nodes and create these complex systems, without any difficult to understand UI or confusing script errors.

This post was more of a rant, and as I now try to teach myself the Unity engine I am reminded by the pain and slog that is working within an engine.
